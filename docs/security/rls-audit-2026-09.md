...
| CI işi `rls-supabase` (GitHub Actions, `supabase/postgres:17.6.1.166`, production ile aynı imaj) | Aynı 42 test gerçek Postgres 17'de, Supabase'in kendi `anon`/`authenticated`/`service_role` rolleri ve `auth.uid()` ile çalışır. Ücretsiz; her PR'da tekrarlanır                       | **42/42 geçti** (PostgreSQL 17.6, rol `postgres`; CI run 35878249991, 23.09.2026) |
